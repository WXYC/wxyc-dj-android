...
- **AGP 9.4.1 and Gradle 9.6.0, held there by two compatibility flags in `gradle.properties`: `android.builtInKotlin=false` and `android.newDsl=false`.** AGP 9 compiles Kotlin itself and then refuses the `org.jetbrains.kotlin.android` plugin outright ([issuetracker 438678642](https://issuetracker.google.com/issues/438678642)); its new DSL's `ApplicationExtension` also no longer extends the `BaseExtension` that Kotlin 2.2.20's KGP casts to when applying that plugin, which is a `ClassCastException` at plugin-apply time. Both flags have the same single root cause — KGP 2.2.20 predates AGP 9 — and this is checkable rather than assumed: `-Pandroid.debug.obsoleteApi=true` reports `kotlin-android` as the caller behind every legacy-variant-API warning in the build, not anything in this repo's build files. AGP removes both flags in 10.0, so **the exit is a Kotlin bump, not a rewrite of these build files.** `android.newDsl=false` is not purely cosmetic, either: under it AGP 9.4.1 **silently ignores `testedAbi`** on a managed device — a value set there never reaches `ManagedDeviceInstrumentationTestSetupTask`, so it neither selects an image nor hard-errors on a mismatch the way AGP's own source says it should. That matters on a deadline, because AGP 10 changes the unspecified default to `arm64-v8a` and would hand the x86 CI runner an ARM test APK; see the comment on the `atdApi30` device and #36. Adopting built-in Kotlin is not an alternative route: `:api` is a pure-JVM module with no AGP to supply a Kotlin compiler at all.
- **Hilt 2.60.1.** This pin previously read "2.58, not 2.59+" because the Hilt Gradle plugin requires AGP 9.0 from 2.59 and the repo was on AGP 8.13.2. The AGP 9 upgrade inverted that constraint, so the pin is retired rather than raised.
- **Coil 3.3.0, not 3.5.0** — later Coil 3 module metadata hard-requires `kotlin-stdlib 2.4.0` against this repo's Kotlin 2.2.20.
- **The `hilt { enableAggregatingTask = false }` workaround is gone, and should not come back without a fresh reproduction.** It worked around a real Hilt Gradle plugin bug (google/dagger#4976/#4048: `NoSuchMethodError: ClassName.canonicalName()`, a JavaPoet version mismatch on the aggregating task's own worker classpath) reproduced against Hilt 2.58. Hilt 2.60.1 fixes it — with the block deleted, `:app:hiltAggregateDepsDebug` runs and `:app:testDebugUnitTest --rerun-tasks` is green.
- **`lint { textReport = true }` stays, despite AGP 9 deprecating it.** The deprecation message says lint reports are "now always generated" and points at `SingleArtifact.LINT_TEXT_REPORT`, which makes the line look safely deletable. It is not: with it removed, `:app:lintDebug --rerun-tasks` prints no findings at all. "Always generated" means written to a file, and the CI job's entire lint signal is stdout — a silent gate is worse than a deprecation warning. Wire the artifact up deliberately as part of the AGP 10 work.
...
### Album detail (issue #10, `ui/detail/`)

`AlbumDetailViewModel` is `@HiltViewModel(assistedFactory = AlbumDetailViewModel.Factory::class)` with `@AssistedInject constructor(@Assisted albumId: Int, apiClient: ApiClient)`, and `AlbumDetailScreen` resolves it via `hiltViewModel<AlbumDetailViewModel, AlbumDetailViewModel.Factory>(creationCallback = { it.create(route.id) })`. This was a deliberate choice over the more common `SavedStateHandle.toRoute<AlbumRoute>()` pattern (which would also work, since `AlbumRoute` is `@Serializable` and Navigation Compose populates a destination's `SavedStateHandle` from its typed-route args automatically): assisted injection makes `albumId` an explicit constructor parameter, so a test constructs the view model directly (`AlbumDetailViewModel(albumId = 100, apiClient = someClient)`) with no `SavedStateHandle`/`NavBackStackEntry` machinery to stand up, rather than depending on that Navigation Compose plumbing actually working the way assumed. `AlbumRouteFallbackStore.take(albumId)` is read once, in `init`, which is what makes the fallback-row hand-off survive a configuration change — the placeholder this replaced read the same store via `remember(route.id)` in the composable, torn down and rebuilt on every rotation.

`ArtworkFailureClassification` (`:api`) classifies a failed Coil image load as indicting the URL or merely the link, mirroring iOS's identically-named type — but the *seam* differs: iOS classifies over `URLError`, a single closed type `URLSession` always throws, while Coil (Android-only) can't be imported into the SDK-free `:api` module. The classification here is over a raw `Throwable`, keyed on the standard `java.net`/`javax.net.ssl` hierarchy, and it walks the `cause` chain rather than matching the top frame — OkHttp reports a connection reset as a bare `IOException` carrying the real `SocketException` as its cause, and matching the top frame alone retires a healthy cover permanently, which is the exact defect the classification exists to prevent. It landed separately, ahead of this screen, so this PR stayed reviewable; see its own KDoc for the full argument and for why "any network exception is transient" is the trap to avoid.

**A MockWebServer gotcha worth knowing before writing another concurrent-fan-out test.** `AlbumDetailViewModelTest` originally enqueued responses in a fixed order (`server.enqueue(...)` per expected request) the way `LoginViewModelTest`/`ApiClientTest` do — and intermittently failed, because invariant 18's "with a fallback" branch fires `/library/info` and `/proxy/metadata/album` **genuinely concurrently** on OkHttp's own thread pool. `MockWebServer`'s default `QueueDispatcher` hands out enqueued responses strictly in **arrival order**, not enqueue-intent order, so two racing requests can and did receive each other's responses under load — visible as a request to `/library/info` decoding as `AlbumMetadata` instead. The fix is a small `Dispatcher` keyed by `request.path` (see `respondByPath` in the test) rather than a queue; any future test with two requests that can race needs the same shape, not another `server.enqueue()` call and a hope about ordering. Also worth knowing: `android.util.Log` calls (this view model logs failures the way `TokenStorageModule` already does) throw `RuntimeException: ... not mocked` under a plain JUnit4 unit test — `AlbumDetailViewModelTest` runs under `@RunWith(RobolectricTestRunner::class)` (`BinCollationParityTest`'s same `@Config(sdk = [34])`) specifically so its failure-leg tests can exercise those lines at all.
