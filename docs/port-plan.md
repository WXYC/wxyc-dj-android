...
| Navigation | `NavigationStack` + `AlbumRoute` | **Navigation Compose, type-safe routes** | `@Serializable` route classes. `AlbumRoute`'s id-only identity maps across but **not for the same reason** (issue #23): `NavigationPath` keys the back stack on a pushed value's `Hashable`, Navigation Compose keys it on the encoded route string. So the property is held by making the route carry nothing but the id, not by writing `equals`/`hashCode` over it — and the row iOS carries as `AlbumRoute.fallback` travels out of band in `AlbumRouteFallbackStore`. |
